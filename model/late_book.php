...
{   
    ...
    private function get_book_is_borrow($no_mhs){
        $denda = new Hitung();
        global $mysqli;
        $query="SELECT a.no_barcode,a.kd_buku,a.tgl_kembali,a.tgl_pinjam,a.tgl_dikembalikan,b.judul FROM transaksi a LEFT JOIN  buku b ON a.`kd_buku` = b.kd_buku  WHERE no_mhs=" . $no_mhs ." AND tgl_dikembalikan = '0000-00-00' ";
        $data = array();
        $result = $mysqli->query($query);
        $num_rows = mysqli_num_rows($result);
